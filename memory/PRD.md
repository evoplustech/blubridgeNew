...
## 2026-06 (fork 4) — "What we can do for you" v5: EDITORIAL HEADER INDEX (current)
- v4 (two-row scattered field + locator) REJECTED. Rebuilt as compact header zone: title (lg cols 1-7, clamp 40-74px, max 620px) + capability selector (lg cols 9-12, two-column plain-text index: SA/AIDS, IDR/DA, CAD spanning; 16.5px, 44px targets, active = navy 600 + 2px text-width underline, inactive #5d6580). Reading stage below (lg col-start-2, 820px max): heading clamp(32-56px) + paragraph 17.5px/660px on one exact left axis, 170ms bbCapReveal. Zero boxes/panels/lines. A11y: tablist/tab/tabpanel, roving tabindex, arrow keys; focus-visible = accent underline on .bb-cap-label (no outline box). Mobile: title → 2-col selector → content, no overflow. Content verbatim, testids preserved. Replaced via python line-swap (203-376). Verified screenshots 1440px (Smart Agents + In-Depth Research) and 390px.

## 2026-06 (fork 4) — "What we can do for you" v4: EDITORIAL CAPABILITY FIELD (SUPERSEDED by v5)
- Aperture (v3) REJECTED. Rebuilt as two-row typographic field: row1 Smart Agents (col1-3) / AI Driven Search (col4-7) / In-Depth Research (col8-12); row2 Developer APIs (col2-5) / Custom AI Deployments (col6-12); labels clamp(19-36px), inactive #5d6580, active navy 600 + 2px text-width underline. Full-width 1px rule beneath with 56px navy locator segment that slides (220ms, .bb-cap-locator, measured via getBoundingClientRect on activeCap/resize) under the selected label. Open reading block (lg col3-8): heading clamp(30-54px, 760px) + paragraph (17px, 660px, mt 30) on one left axis, 170ms bbCapReveal. A11y unchanged (tablist/tab/tabpanel, roving tabindex, arrow keys, focus-visible). Mobile: 2-col label grid, CAD spans both. Content verbatim, testids preserved. GOTCHA: a giant single search_replace on this component previously corrupted the file (duplicated tail + silent revert) — line-based python replacement used instead; verify grep after big JSX swaps. Verified via screenshots 1440px (default + Developer APIs) and 390px.

## 2026-06 (fork 4) — "What we can do for you" v3: CAPABILITY APERTURE (SUPERSEDED by v4)
- Prior reading-frame layout REJECTED (repeated left-nav/right-content pattern of infra section above). Rebuilt as expanding horizontal aperture: one continuous field (shared top/bottom rules + 1px internal separators, lg height 500px), all 5 capabilities always visible; active aperture flex-grows to ~54% (320ms ease-out via .bb-aperture CSS in index.css) showing label + 30px rule + heading clamp(27-46px, 620px max) + paragraph (16.5px, 600px, mt 28, same left axis); inactive = narrow typographic columns (15.5px muted, wrapping labels). <lg: stacked full-width disclosure rows, active expands beneath its own label. A11y: tablist/tab/tabpanel, roving tabindex, arrow keys, :focus-visible outline, prefers-reduced-motion kills transitions. Content verbatim; NOTE: tabs 3/4/5 share identical heading string — ORIGINAL site copy, mapping is correct, content-lock forbids rewrite. Testids preserved. Verified via screenshots 1440px (default + In-Depth Research expanded) and 390px (no overflow).

## 2026-06 (fork 4) — "What we can do for you" Capability Reading Frame (SUPERSEDED by v3 above)
