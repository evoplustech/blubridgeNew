...
- `Footer.jsx`: new layout matching user screenshot on ALL pages (route-aware /contact bg variant removed; uniform #e8eaf3). Top grid: brand block (logo 22px + privacy note, col 1-5) with CONTACT US boxed form beneath (white 6px-radius inputs: First/Last row, Email, Message optional, full-width navy Contact Now, max 486px); PRODUCT (Coming Soon) / SOLUTIONS (3 links) / COMPANY (6 links) as vertical columns with mono caps labels. Hairline + baseline (copyright left, Privacy Policy & Terms Of Use right). All links, form logic, and testids (footer-first-name/last-name/email/message/submit, site-footer) preserved. Verified via screenshots on Home and Careers.
