...
import { stages, timelines, budgetTypes, budgets, budgetStatuses, budgetLabel } from '../ai-consulting/options';
import { DropdownField } from './DropdownField';

export const ProjectDetails = ({ state }) => {
  const { form: project, errors: projectErrors, change: changeProject } = state;
  const props = name => ({ name, value: project[name], error: projectErrors[name], onChange: changeProject, disabled: state.disabled });
  return <div className="aic-groups" data-testid="aic-project-details">
    <div className="aic-two-columns" data-testid="aic-stage-timeline-row">
      <DropdownField {...props('stage')} label="What is the current stage of your project?" placeholder="Select your project stage" options={stages} />
      <DropdownField {...props('timeline')} label="When would you like to start?" placeholder="Select your expected start timeline" options={timelines} />
    </div>
    <div className="aic-budget-section" data-testid="aic-budget-section">
      <fieldset className="aic-fieldset" aria-describedby={`aic-budget-type-helper${projectErrors.budgetType ? ' aic-budget-type-error' : ''}`} aria-invalid={Boolean(projectErrors.budgetType)}>
        <legend className="aic-label" data-testid="aic-budget-type-label">How would you like to specify your budget?<RequiredMark /></legend>
        <div className={`aic-budget-types${projectErrors.budgetType ? ' aic-radio-invalid' : ''}`}>
          {budgetTypes.map(([value, label]) => <label className="aic-choice" key={value} data-testid={`aic-budget-type-label-${value}`}>
      ...
      <div className="aic-two-columns" data-testid="aic-budget-status-row">
        <DropdownField {...props('estimatedBudget')} label={budgetLabel(project.budgetType)} placeholder="Select your estimated budget" options={budgets[project.budgetType] || []} disabled={state.disabled || !project.budgetType} />
        <DropdownField {...props('budgetStatus')} label="What is the current status of this budget?" placeholder="Select budget status" options={budgetStatuses} />
      </div>
    </div>
  </div>;
};