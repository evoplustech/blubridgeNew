  ...
  <fieldset aria-labelledby="aic-services-label" aria-describedby="aic-services-help" data-testid="aic-services-group">
    ...
    <div className="aic-service-grid">
      {serviceOptions.map(({ id, value, key, label, field }) => {
        ...
        const panelId = `aic-4-service-panel-${id}`;
        return <div key={id} className={`aic-v4-service${id === 'other' ? ' aic-v4-service-other' : ''}${selected ? ' aic-v4-service-selected' : ''}${state.errors.services || (selected && error) ? ' aic-service-invalid' : ''}`} data-testid={`aic-4-service-item-${id}`}>
          ...
          {selected && <div id={panelId} className="aic-v4-service-panel" data-testid={`aic-4-service-panel-${id}`}>
