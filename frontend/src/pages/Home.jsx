...
// Inference Clusters Visual Component - Cinematic Hero Animation
const InferenceClustersVisual = () => {
  ...
  useEffect(() => {
    ...
    const drawGlow = (x, y, radius, color, intensity) => {
      ...
      gradient.addColorStop(0.5, color.replace(')', `, ${intensity * 0.3})`).replace('rgb', 'rgba'));
    ...
    const animate = () => {
      ...
      ambientGradient.addColorStop(0, 'rgba(30, 80, 150, 0.08)');
      ambientGradient.addColorStop(0.5, 'rgba(20, 50, 100, 0.04)');
      ...
      for (let i = 0; i < clusters.length - 1; i++) {
        ...
        for (let p = 0; p < particleCount; p++) {
          const progress = ((time * 0.3 + p * 0.33 + i * 0.15) % 1);
          const px = c1.x + (c2.x - c1.x) * progress;
          const py = c1.y + (c2.y - c1.y) * progress;
          const particleAlpha = Math.sin(progress * Math.PI) * 0.8;
          
          drawGlow(px, py, 8, 'rgb(100, 180, 255)', particleAlpha * 0.5);
          ctx.beginPath();
      ...
      clusters.forEach((cluster, clusterIndex) => {
        const clusterPulse = Math.sin(time * 0.5 + cluster.basePhase) * 0.2 + 0.8;
        
        // Cluster ambient glow
        drawGlow(cluster.x, cluster.y, 80, 'rgb(40, 100, 180)', 0.15 * clusterPulse);
        
        // Draw internal connections within cluster
        cluster.nodes.forEach((node1, i) => {
          const n1x = cluster.x + node1.offsetX;
          const n1y = cluster.y + node1.offsetY;
          
          cluster.nodes.forEach((node2, j) => {
            if (j <= i) return;
            const n2x = cluster.x + node2.offsetX;
            const n2y = cluster.y + node2.offsetY;
            const dist = Math.sqrt((n2x - n1x) ** 2 + (n2y - n1y) ** 2);
            
            if (dist < 50) {
              const alpha = (1 - dist / 50) * 0.3 * clusterPulse;
              ctx.beginPath();
              ctx.moveTo(n1x, n1y);
              ctx.lineTo(n2x, n2y);
        ...
        cluster.nodes.forEach((node) => {
          const nx = cluster.x + node.offsetX;
          const ny = cluster.y + node.offsetY;
          const nodePulse = Math.sin(time * node.pulseSpeed + node.phase) * 0.3 + 0.7;
          const nodeRadius = node.radius * nodePulse;
          
          // Node glow
          drawGlow(nx, ny, nodeRadius * 6, 'rgb(60, 140, 220)', 0.3 * nodePulse);
          
          // Node core with gradient
          const nodeGradient = ctx.createRadialGradient(nx, ny, 0, nx, ny, nodeRadius);
          nodeGradient.addColorStop(0, `rgba(180, 220, 255, ${0.9 * nodePulse})`);
          nodeGradient.addColorStop(0.5, `rgba(80, 160, 240, ${0.8 * nodePulse})`);
          nodeGradient.addColorStop(1, `rgba(40, 100, 180, ${0.6 * nodePulse})`);
        ...
        const coreRadius = 6 + Math.sin(time + cluster.basePhase) * 2;
        drawGlow(cluster.x, cluster.y, 30, 'rgb(100, 180, 255)', 0.4 * clusterPulse);
        
        const coreGradient = ctx.createRadialGradient(cluster.x, cluster.y, 0, cluster.x, cluster.y, coreRadius);
        coreGradient.addColorStop(0, 'rgba(200, 230, 255, 0.95)');
        coreGradient.addColorStop(0.5, 'rgba(100, 180, 255, 0.8)');
        coreGradient.addColorStop(1, 'rgba(60, 140, 220, 0.6)');
      ...
      // Add subtle violet accent glows
      const violetPositions = [
        { x: width * 0.2, y: height * 0.3 },
        { x: width * 0.8, y: height * 0.7 }
      ];
      violetPositions.forEach((pos, i) => {
        const violetPulse = Math.sin(time * 0.3 + i * 2) * 0.3 + 0.5;
        drawGlow(pos.x, pos.y, 100, 'rgb(120, 80, 180)', 0.05 * violetPulse);
      });
  ...
  return (
    <div className="relative w-full" style={{ height: '500px' }}>
      ...
      />
      {/* Text overlay area - left side for hero text */}
        ...
        <div className="container-custom">
          <div className="max-w-xl">
            ...
            <p className="text-white/70 text-lg leading-relaxed mb-6">
              Scalable AI inference infrastructure with interconnected compute clusters, delivering high-performance model serving at enterprise scale.
            </p>
            <Link to="/products/inference" className="inline-flex items-center gap-2 text-[#60a5fa] hover:text-[#93c5fd] transition-colors font-medium">
