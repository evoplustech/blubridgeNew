...
import { countries, countryByCode, filterCountry } from './countryData';
...
export const CountrySelect = ({ id, value, onChange, disabled, error, callingCode = false }) => {
  ...
  return <Popover open={open && !disabled} onOpenChange={setOpen}>
    <PopoverTrigger asChild>
      <Button id={id} type="button" variant="outline" role="combobox" aria-expanded={open} aria-haspopup="dialog" aria-controls={open ? `${id}-popover` : undefined} aria-label={callingCode ? 'Country calling code' : 'Your Country / Region'} title={selected?.name} aria-required={!callingCode} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} disabled={disabled} className={`aic-input aic-country-trigger${error ? ' aic-invalid' : ''}${selected ? '' : ' aic-placeholder'}`} data-testid={id}>
    ...
    <PopoverContent id={`${id}-popover`} className="aic-country-popover" align="start" collisionPadding={16} aria-label={callingCode ? 'Select calling code' : 'Select country or region'} data-testid={`${id}-popover`}>
      <Command filter={filterCountry}>
        ...
        <CommandList aria-label={callingCode ? 'Country calling codes' : 'Countries and regions'} data-testid={`${id}-list`}>
