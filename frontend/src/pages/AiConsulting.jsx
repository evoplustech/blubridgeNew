...
import { ContactDetails } from './ai-consulting-base/ContactDetails';
import { RequirementsSection } from './ai-consulting/RequirementsSection';
import { ProjectDetails } from './ai-consulting-base/ProjectDetails';
...
import './ai-consulting-base/Dropdowns.css';
...
export default function AiConsulting() {
  ...
  useDocumentTitle('AI Consulting Services | Tailored AI Systems | BluBridge');
  ...
  return <section className="aic-page aic-base-smooth-page" data-testid="ai-consulting-page">
    <div className="aic-container">
      ...
      <form className="aic-form-surface" method="post" noValidate onSubmitCapture={event => event.preventDefault()} onSubmit={state.handleSubmit} aria-busy={state.isSubmitting} aria-labelledby="aic-form-heading" data-testid="aic-form">
        ...
        <fieldset className="aic-submit-fields" disabled={state.disabled}>
          ...
          <section className="aic-form-section" aria-label="Contact permission" data-testid="aic-section-permission">
            <label className="aic-choice aic-permission" htmlFor="aic-contact-permission" data-testid="aic-contact-permission-label">
              ...
              <span className="aic-base-consent-copy" data-testid="aic-consent-copy">I acknowledge BluBridge’s Privacy Policy and consent to being contacted regarding my enquiry. I acknowledge the <span className="aic-base-policy-reference" data-testid="aic-policy-reference"><Link to="/policies/privacy-policy" data-testid="aic-privacy-link">Privacy Policy</Link>.<RequiredMark /></span></span>
            </label>
            <FieldError name="contactPermission" error={state.errors.contactPermission} />
          </section>
          <div className="aic-actions"><Button type="submit" className="aic-primary" disabled={state.disabled} aria-busy={state.isSubmitting} data-testid="aic-submit-enquiry">
            {state.isSubmitting && <Loader2 className="aic-base-submit-spinner" aria-hidden="true" data-testid="aic-submit-spinner" />}
            <span aria-live="polite" data-testid="aic-submit-label">{state.receipt ? 'Enquiry Submitted' : state.isSubmitting ? 'Submitting…' : 'Submit Enquiry'}</span>
          </Button></div>
        </fieldset>
